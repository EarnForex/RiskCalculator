...
//|          Main difference with MT4 OrderMap - nodes are stored    |
//|          sorted by ticket after being sorted by price.           |
//+------------------------------------------------------------------+
...
public:
    ...
    CDOMObject      *GetNodeByTicket(ulong order_ticket);
    ...
    else
    {
        ...
        for (node = GetFirstNode(), m_curr_idx = 0; node != NULL; node = GetNextNode(), m_curr_idx++)
        {
            ...
            if (node.Price() < new_node.Price())
            {
                ...
                // No need to check for ticket sorting here.
                ...
                {
                    // Check if previous node has the same price as the new node.
                    // If it does, cycle back while prev.ticket > new_node.ticket.
                    while ((node.Prev() != NULL) && (node.Prev().Price() == new_node.Price()) && (node.Prev().Ticket() > new_node.Ticket()))
                    {
                        node = node.Prev();
                        m_curr_idx--;
                    }
...
//+------------------------------------------------------------------+
CDOMObject *COrderMap::GetNodeByTicket(ulong order_ticket)
{
    if (!CheckPointer(m_first_node)) return NULL;
