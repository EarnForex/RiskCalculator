...
public:
   ...
   COrderIterator(void): Status(NULL), StatusHedging(NULL), RO(NULL), current_price(0), unrealized_profit(0), realized_profit(0), mode(Risk) {if (hedging) {StatusHedging = new COrderMap;} RO = new COrderMap;}
  ~COrderIterator(void);
...
{
   RO = new COrderMap; StatusHedging = new COrderMap;
   CDOMObject *order, *new_order;
